...
│       ├── datasources/loki.yaml           # Loki 数据源自动注册
│       └── dashboards/
│           ├── dashboard-provider.yaml     # Dashboard 加载器
│           └── json/smart-cs-logs.json     # 预置日志监控 Dashboard
├── Dockerfile              # 多阶段构建（eclipse-temurin:21-alpine）
├── docker-compose.yml      # 全栈编排（PG + Redis + App + Loki + Promtail + Grafana）
├── .env                    # 本地开发环境变量（不提交）
...
| Grafana | `http://<host>:3000` | 日志可视化（仅本机可访问） |
...
**日志链路：** App JSON stdout → Promtail（Docker SD 采集）→ Loki（tsdb 存储，30天保留）→ Grafana（可视化）
...
**Prometheus 指标：** JVM 内存/GC、HikariCP 连接池、HTTP 请求 QPS/延迟、自定义业务指标。
